...
const ALL_WIDGET_IDS: WidgetId[] = [
  ...
  'raidSchedule', 'raidCalendar', 'guildIntro',
]
...
export default function LayoutBuilder({ guildId, guildCode, initialLayout }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(e && e.message ? e.message : '저장에 실패했습니다.')
      setSaving(false)
    }
  }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 gap-2">
          <button
            onClick={resetLayout}
            className="rounded-lg border border-zinc-800 px-3 py-2 text-sm text-zinc-400 transition hover:bg-zinc-800/60"
          >
            되돌리기
          </button>
          <button
          ...
          return (
            <div
              ...
              className={
                ...
                (overCol === c.key ? 'border-violet-500/60 bg-violet-500/5' : 'border-zinc-800 bg-zinc-900/40')
              }
            >
              <p className="mb-2 font-mono text-[11px] uppercase tracking-wider text-zinc-500">
