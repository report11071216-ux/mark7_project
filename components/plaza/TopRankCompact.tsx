...
function RankCard(props: { guild: RankedGuild; rank: number }) {
  ...
  const cardClass = isFirst
    ? "block p-4 transition hover:bg-blue-50 bg-gradient-to-b from-yellow-50 to-transparent"
    : "block p-4 transition hover:bg-blue-50";
    ...
    <Link href={"/guild/" + guild.code} className={cardClass}>
      ...
      <div className="flex items-center gap-2 mb-2">
        {guild.logo_url ? (
          ...
          <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600 text-sm font-bold shrink-0">
            {guild.name.charAt(0)}
          </div>
        )}
